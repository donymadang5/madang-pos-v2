...
module.exports = async (sock, jid, body) => {
    ...
    if (await home(sock,jid,body,state)) return true;
    if (await verify(sock,jid,body,state)) return true;
    if (await customer(sock,jid,body,state)) return true;
    if (await order(sock,jid,body,state)) return true;
    if (await statistic(sock,jid,body,state)) return true;
    if (await broadcast(sock,jid,body,state)) return true;
