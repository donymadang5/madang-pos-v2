    ...
    try {
        ...
        const products = rows
            .filter(row =>
                ...
                !String(row.kode_barang_edit).startsWith("Data Kolom")
            )
            .map(row => ({
                id: String(row.kode_barang_edit),
                nama: row.nama_barang_edit || "",
                harga: Number(row.harga_jual_edit || 0),
                stok: Number(row.stok_edit || 0),
        ...
        await activity.add(
            "IMPORT",
            `${jid} mengimpor ${products.length} produk`,
            {
                admin: jid,
...
📝 Activity berhasil disimpan.`
        ...
        await sock.sendMessage(jid, {
            text: "❌ Gagal mengimpor file Excel."
        });
    }
};
